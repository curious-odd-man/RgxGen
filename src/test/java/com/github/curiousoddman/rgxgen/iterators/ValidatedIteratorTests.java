...
public class ValidatedIteratorTests {
    ...
    public void next_returnsNext_whenThereAreMoreElements() {
        aValidatedIterator = new ValidatedIterator(Collections.singletonList(aValidator), new TestIterator("A", "B", "C"));

        when(aValidator.validate(anyString())).thenReturn(true);
    ...
    public void next_throws_whenNoMoreElements() {
        aValidatedIterator = new ValidatedIterator(Collections.singletonList(aValidator), new TestIterator());
        aValidatedIterator.next();
    }

    @Test(expected = NoSuchElementException.class)
    public void next_throws_afterFirstElement() {
        aValidatedIterator = new ValidatedIterator(Collections.singletonList(aValidator), new TestIterator("A"));
    ...
    public void next_throws_whenThereAreOnlyInvalidElementsLeft() {
        aValidatedIterator = new ValidatedIterator(Collections.singletonList(aValidator), new TestIterator("A", "B", "C"));
        when(aValidator.validate(anyString())).thenReturn(false);
        aValidatedIterator.next();
    }
    ...
    public void hasNext_returnsFalse_whenThereAreOnlyInvalidElementsLeft() {
        aValidatedIterator = new ValidatedIterator(Collections.singletonList(aValidator), new TestIterator("A", "B"));
    ...
    public void reset_reStartsFromBeginning() {
        ...
        when(aValidator.validate(eq("A"))).thenReturn(true);
        when(aValidator.validate(eq("B"))).thenReturn(false);
        when(aValidator.validate(eq("C"))).thenReturn(true);
